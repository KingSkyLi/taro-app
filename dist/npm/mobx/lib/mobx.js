...
function assertPropertyConfigurable$$1(object, prop) {
  if (!isPropertyConfigurable$$1(object, prop)) fail$$1("Cannot make property '" + prop.toString() + "' observable, it is not configurable and writable in the target object");
}
...
function createPropDecorator$$1(propertyInitiallyEnumerable, propertyCreator) {
  return function decoratorFactory() {
    ...
    // as the instance to apply the decorator to equals the target
      ...
      if (!quacksLikeADecorator$$1(arguments)) fail$$1("This function is a decorator, but it wasn't invoked like a decorator");
...
function refStructEnhancer$$1(v, oldValue, name) {
  if (isObservable$$1(v)) throw "observable.struct should not be used with observable values";
...
function endAction(runInfo) {
  ...
  if (runInfo.notifySpy && true) spyReportEnd$$1({ time: Date.now() - runInfo.startTime });
}
...
var ObservableValue$$1 = /** @class */function (_super) {
  ...
  function ObservableValue$$1(value, enhancer, name, notifySpy) {
    ...
    if (notifySpy && isSpyEnabled$$1() && true) {
  ...
  ObservableValue$$1.prototype.set = function (newValue) {
    ...
    if (newValue !== globalState$$1.UNCHANGED) {
      ...
      if (notifySpy && true) spyReportEnd$$1();
    }
  };
...
var ComputedValue$$1 = /** @class */function () {
  ...
  function ComputedValue$$1(options) {
    ...
    if (!options.get) throw "[mobx] missing option for computed: get";
  ...
  ComputedValue$$1.prototype.set = function (value) {
    ...
    } else invariant$$1(false, "[ComputedValue '" + this.name + "'] It is not possible to assign a new value to a computed value.");
  };
  ComputedValue$$1.prototype.trackAndCompute = function () {
    if (isSpyEnabled$$1() && true) {
      spyReport$$1({
        object: this.scope,
        type: "compute",
        name: this.name
...
function checkIfStateModificationsAreAllowed$$1(atom) {
  ...
  if (globalState$$1.computationDepth > 0 && hasObservers$$1) fail$$1("Computed values are not allowed to cause side effects by changing observables that are already being observed. Tried to modify: " + atom.name);
  // Should not be possible to change observed state outside strict mode, except during initialization, see #563
  if (!globalState$$1.allowStateChanges && (hasObservers$$1 || globalState$$1.enforceActions === "strict")) fail$$1((globalState$$1.enforceActions ? "Since strict-mode is enabled, changing observed observable values outside actions is not allowed. Please wrap the code in an `action` if this change is intended. Tried to modify: " : "Side effects like changing state are not allowed at this point. Are you trying to modify state from, for example, the render function of a React component? Tried to modify: ") + atom.name);
}
/**
...
function propagateChanged$$1(observable$$1) {
  ...
  if (observable$$1.lowestObserverState === exports.IDerivationState.STALE) return;
...
function propagateMaybeChanged$$1(observable$$1) {
  ...
  if (observable$$1.lowestObserverState !== exports.IDerivationState.UP_TO_DATE) return;
  ...
  Reaction$$1.prototype.runReaction = function () {
    if (!this.isDisposed) {
      ...
      if (shouldCompute$$1(this)) {
        ...
        try {
          this.onInvalidate();
          if (this._isTrackPending && isSpyEnabled$$1() && true) {
...
function decorate$$1(thing, decorators) {
  ...
  var _loop_1 = function (prop) {
    ...
    invariant$$1(propertyDecorators.every(function (decorator) {
...
function extendObservableObjectWithProperties$$1(target, properties, decorators, defaultDecorator) {
  ...
  try {
    for (var key in properties) {
      ...
      if (typeof decorator !== "function") fail$$1("Not a valid decorator for '" + key + "', got: " + decorator);
...
function _isObservable(value, property) {
  if (value === null || value === undefined) return false;
  if (property !== undefined) {
    if (isObservableMap$$1(value) || isObservableArray$$1(value)) return fail$$1("isObservable(object, propertyName) is not supported for arrays and maps. Use map.has or array.length instead.");
...
var ObservableArrayAdministration = /** @class */function () {
  ...
  ObservableArrayAdministration.prototype.notifyArrayChildUpdate = function (index, newValue, oldValue) {
    ...
    if (notifySpy && true) spyReportStart$$1(__assign({}, change, { name: this.atom.name }));
...
var ObservableMap$$1 = /** @class */function () {
  ...
  ObservableMap$$1.prototype.delete = function (key) {
    ...
    if (this._has(key)) {
      ...
      if (notifySpy && true) spyReportStart$$1(__assign({}, change, { name: this.name, key: key }));
...
function getAtom$$1(thing, property) {
  if (typeof thing === "object" && thing !== null) {
    ...
    if (isObservableMap$$1(thing)) {
      ...
      if (!observable$$1) fail$$1("the entry '" + property + "' does not exist in the observable map '" + getDebugName$$1(thing) + "'");
      return observable$$1;
    }
    // Initializers run lazily when transpiling to babel, so make sure they are run...
    initializeInstance$$1(thing);
    ...
    if (isObservableObject$$1(thing)) {
      ...
      if (!observable$$1) fail$$1("no observable property '" + property + "' found on the observable object '" + getDebugName$$1(thing) + "'");
      return observable$$1;
    }
...
(function () {
  ...
  if (testCodeMinification.name !== "testCodeMinification" && true) {
    console.warn(
